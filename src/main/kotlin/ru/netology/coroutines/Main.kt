        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            PostWithComments(post.id, getAuthor(client, post.id).name, getAuthor(client, post.id).avatar,
                                post.content, post.likedByMe, post.likes, getComments(client, post.id).map{
                                    comment -> async {
                                        CommentWithAuthor(comment.id, comment.postId, getAuthor(client, comment.authorId).name,
                                            getAuthor(client, comment.authorId).avatar, comment.content, comment.published,
